...
```mermaid
flowchart LR
  A["Pause one beat"] --> B["Bridge to a legitimate topic"]
  B --> C["Pivot to the real signal"]
  C --> D["Continue the round"]
```
*The four-move deflection sequence for an illegal or near-illegal question.*
...
```mermaid
flowchart TD
  Q["Illegal or near-illegal question asked"] --> R{"Single and asked in good faith"}
  R -->|"Yes"| N["Deflect and log it in the after-action notes"]
  R -->|"No - patterned or hostile"| E["Consider escalation"]
  E --> P1["Path 1 - tell the recruiter mid loop"]
  E --> P2["Path 2 - tell HR post loop"]
  E --> P3["Path 3 - decline the offer and write a letter"]
  E --> P4["Path 4 - file an EEOC charge"]
```
*From a single good-faith question that gets deflected to a patterned or hostile one that moves through the escalation paths.*
