...
```mermaid
flowchart TD
    DAU["DAU times actions per user"] --> AvgQPS["Average QPS"]
    AvgQPS -->|times 3 to 5| PeakQPS["Peak QPS"]
    DAU --> Records["Records per day"]
    Records -->|times record size times 365| Storage["Storage per year"]
    PeakQPS -->|times payload size| Bandwidth["Bandwidth"]
```
*The four-step estimation walk — one DAU number fans out into peak QPS, storage, and bandwidth.*
...
sequenceDiagram
    ...
    Client->>CDN: request short URL
    alt CDN cache hit
        CDN-->>Client: redirect about 21 ms
    else CDN cache miss
        CDN->>LB: forward request
        LB->>App: route request
        App->>Cache: lookup short code
        Cache-->>App: cached value
        App-->>CDN: redirect response
        CDN-->>Client: redirect about 28 to 33 ms
    end
```
*Each hop on the redirect path adds latency; a CDN hit skips most of the chain.*
