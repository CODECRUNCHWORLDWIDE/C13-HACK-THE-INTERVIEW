...
```mermaid
flowchart TD
  Total["30 target companies"] --> A["Tier A Reaches - 10 companies"]
  Total --> B["Tier B Matches - 15 companies"]
  Total --> C["Tier C Safeties - 5 companies"]
  A --> AO["1 to 3 expected offers"]
  B --> BO["3 to 6 expected offers"]
  C --> CO["2 to 4 expected offers"]
```
*The 30-company list splits into three tiers, each with its own realistic offer yield.*
