...
flowchart TD
  ...
  C --> D["Field parsing"]
  D --> E["Keyword indexing"]
  E --> F["Candidate record"]
  F --> G["Recruiter queue sorted by JD match"]
```
*The four-stage ATS pipeline a resume passes through before a recruiter ever sees it.*
...
```mermaid
flowchart TD
  A["Candidate profile"] --> B["New grad or career changer"]
  A --> C["Most candidates"]
  A --> D["Senior plus"]
  B --> B1["Header then Education then Projects then Experience then Skills"]
  C --> C1["Header then Skills then Experience then Projects then Education"]
  D --> D1["Header then Summary then Experience then Skills then Education"]
```
*Section order changes with candidate profile — the header always leads, everything else reorders around it.*
