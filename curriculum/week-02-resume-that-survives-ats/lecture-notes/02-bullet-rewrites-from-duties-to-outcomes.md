...
```mermaid
flowchart TD
  A["Read the bullet opener"] --> B{"Weak opener - Responsible Worked Helped Participated Assisted or Contributed"}
  B -->|Yes| C["Duties bullet - rewrite or cut"]
  B -->|No| D["Keep and check for a quantified number"]
```
*A quick spot-check for whether an opening word signals a duties bullet.*
...
```mermaid
flowchart LR
  S["Situation optional context"] --> T["Task what needed to happen"]
  T --> A["Action what you specifically did"]
  A --> R["Result the outcome"]
  R --> RR["Plus R the quantified result"]
  RR --> B["One compressed bullet sentence"]
```
*STAR plus R compresses five parts into a single quantified bullet.*
